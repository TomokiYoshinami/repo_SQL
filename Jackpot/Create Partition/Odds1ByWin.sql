...
ALTER TABLE [dbo].[Odds1ByWin] DROP CONSTRAINT [PK_Odds1ByWin]


ALTER TABLE [dbo].[Odds1ByWin] ADD  CONSTRAINT [PK_Odds1ByWin] PRIMARY KEY NONCLUSTERED 
(
	[RaceID] DESC,
	[BetTypeCD] ASC,
	[Kumi1] ASC,
...
CREATE CLUSTERED INDEX [ClusteredIndex_on_PS_Date_636027171993494074] ON [dbo].[Odds1ByWin]
(
	[RaceDate]
)WITH (SORT_IN_TEMPDB = ON, DROP_EXISTING = OFF, ONLINE = OFF) ON [PS_Date]([RaceDate])


DROP INDEX [ClusteredIndex_on_PS_Date_636027171993494074] ON [dbo].[Odds1ByWin]
...
CREATE UNIQUE NONCLUSTERED INDEX [IX_Odds1ByWin] ON [dbo].[Odds1ByWin]
(
	[RaceDate] DESC,
	[JyoCD] ASC,
	[Kaiji] ASC,
